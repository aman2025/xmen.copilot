...
class Controller {
  ...
  async initOrLoadTask(chatId = null, userInput = null) {
    ...
    } else if (userInput) { // Initialize new task
      try {
  ...
  async handleUserMessage(text) {
    ...
    if (this.task && this.currentChatId) { // Sending message to existing, loaded task
  ...
  async handleUserResponse(response) {
    ...
    const userActionText = response === 'approve' ? 'User approved tool execution.' : 'User rejected tool execution.'
